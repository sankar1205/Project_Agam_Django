...
export default function ClientLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        if (data.role !== 'client') {
          setError('Unauthorized: Not a client account.');
          return;
        }
        login(data.access);
  ...
  return (
    <Box sx={{ minHeight: '100vh', display: 'flex', alignItems: 'center', background: 'linear-gradient(135deg, #10b981 0%, #047857 100%)' }}>
        ...
        <Paper elevation={24} sx={{ p: 5, borderRadius: 4, background: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(10px)' }}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <Button 
              ...
              sx={{ mt: 3, mb: 2, borderRadius: 2, py: 1.5, background: 'linear-gradient(45deg, #10b981, #047857)', boxShadow: '0 4px 14px 0 rgba(16, 185, 129, 0.39)', transition: 'all 0.2s', '&:hover': { transform: 'translateY(-2px)' } }}
            >
              Sign In
            </Button>
            <Box textAlign="center" mt={2}>
              <Typography variant="body2">
                Don't have an account? <Link to="/client/signup" style={{ color: '#10b981', fontWeight: 600, textDecoration: 'none' }}>Create Account</Link>
              </Typography>
            </Box>
          </form>
        </Paper>
